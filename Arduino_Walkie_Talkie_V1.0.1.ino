...
void setup() 
{
  ...
  //rfAudio.setVolume(7);       // Set it up to Max Volume
};

void loop() 
{  
  rfAudio.handleButtons();
  // Audio playback is all managed internally. In this example, the radio is controlled
  // by external buttons, so there is nothing to do here except handle the buttons. 
};
